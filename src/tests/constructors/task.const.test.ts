...
describe('Task constructor', () => {
  ...
  it('should throw Creation Error if instructions is missing', () => {
    const data = { ...mockTaskRequestData };
    data.instructions = ''; // TODO: Si le pongo una opción de tipo no válida, debería lanzar un error
    expect(() => new Task(data)).toThrow('The "instructions" field is required.');
  });
